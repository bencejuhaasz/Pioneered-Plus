...
//      * Beat Jump Mode
//      * Sampler Mode
//	* PADFX1 (Uses first 4 effect chains. Applies them either 40 or 60 percent.)
//	* PADFX2 (Beatlooproll)
//	* BeatFX (with 1st effect parameter control besides meta knob)
//	* KeyShift
//      * KeyBoard
//
	...
	if (value) {
      ...
      if(group.includes("Channel2")) {
        midi.sendShortMsg(0x99, control, 0x7F);
			...
			engine.setValue("[EffectRack1_EffectUnit"+unit+"]","loaded_chain_preset",preset);
      ...
      engine.setValue("[EffectRack1_EffectUnit"+unit+"_Effect2]", "meta_toggle", 1);
		...
		if(group.includes("Channel1")) {
      midi.sendShortMsg(0x97, control, 0x00);
    }
    if(group.includes("Channel2")) {
      midi.sendShortMsg(0x99, control, 0x00);
    }
	}
};
